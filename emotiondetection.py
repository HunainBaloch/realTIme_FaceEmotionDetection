    ...
    for (x, y, w, h) in faces:
        ...
        if w > 30 and h > 30:  # Adjust sizes as necessary
            ...
            try:
                ...
                results = DeepFace.analyze(face_frame, actions=['age', 'gender', 'emotion', 'race'], enforce_detection=False)
                if isinstance(results, list) and len(results) > 0:
                    result = results[0]  # Access the first result if it's a list of results

                    emotion_text = result['dominant_emotion']
                    age_text = str(result['age'])
                    gender_text = result['dominant_gender']
                    race_text = result['dominant_race']

                    info_text = f"Emotion: {emotion_text} | Age: {age_text} | Gender: {gender_text} | Race: {race_text}"
                    cv2.putText(frame, info_text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (36, 255, 12), 2)
                else:
                    print("Results are not in expected format:", results)
